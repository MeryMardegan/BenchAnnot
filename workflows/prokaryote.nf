...
workflow PROKARYOTE_ANNOTATION {
    ...
    bakta:  tuple(sample_id, genome_fasta, species, taxid, genetic_code)
    eggnog: tuple(sample_id, genome_fasta)
    pgap:   tuple(sample_id, genome_fasta, species, taxid, genetic_code)
    }
    .set { inputs }
    ...
    PGAP(inputs.pgap, pgap_dir_ch, pgap_container_ch)
}